extern uint32_t GP22_registers_base[7];
...
namespace testReg {
	...
	inline volatile uint8_t *regAddress(GP22_registers reg)
	{
		return reinterpret_cast<volatile uint8_t*>(&GP22_registers_base + 
				reg * sizeof(uint32_t));
	}

	inline uint8_t regRead(GP22_registers reg)
	{
		return *regAddress(reg);
	}
	 
	inline void regWrite(GP22_registers reg, uint8_t value)
	{
