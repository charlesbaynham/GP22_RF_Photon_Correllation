...
void setup() {
  ...
  h.registerCommand(COMMANDHANDLER_HASH("put"), -1, &storeCommand);
  h.registerCommand(COMMANDHANDLER_HASH("get"), 0, &readCommand);
  h.registerCommand(COMMANDHANDLER_HASH("clear"), 0, &clearCommand);
  CommandHandlerReturn r = h.registerCommand(COMMANDHANDLER_HASH("echo"), -1, &echoMany);
...
void readCommand(const ParameterLookup& params) {
  ...
  char str[COMMAND_SIZE_MAX];
  h.getStartupCommand(str);
